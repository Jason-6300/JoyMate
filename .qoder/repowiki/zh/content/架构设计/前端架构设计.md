...
### 专家视角与卡片渲染（流程图）
主内容区根据返回数据决定呈现阶段（专家视角→主持人语→推荐卡片），并使用动画组件实现逐字打字与分组入场效果。

```mermaid
flowchart TD
Start(["开始"]) --> CheckData["检查返回数据"]
CheckData --> HasPersonas{"存在专家视角?"}
HasPersonas --> |是| ShowPersonas["逐字呈现专家观点"]
...
- 外部依赖：@google/genai、openai、lucide-react、motion、react-markdown。
...
Pkg --> Markdown["react-markdown@^10.1.0"]
...
- 状态管理与重渲染
  - 使用useMemo/useCallback稳定引用，减少子组件重渲染（如TypewriterText、RecCard等）。
  - 会话记忆存储于sessionStorage，避免每次刷新丢失用户偏好。
- 动画与滚动
  - 使用motion与AnimatePresence实现流畅过渡，但注意控制动画数量与复杂度，避免掉帧。
...
## 结论
本项目以Next.js app目录为核心，结合React 19的特性与Tailwind CSS样式体系，构建了清晰的三层架构：布局层、页面层与组件层。通过服务封装与API路由，实现了前后端协作的高效流程；通过会话记忆、动画与滚动优化，提升了用户体验。建议在后续迭代中进一步细化代码分割、完善错误边界与监控埋点，持续优化首屏性能与交互流畅度。

## 附录
- Next.js配置：启用严格模式与自定义构建目录，便于开发与部署。
- 依赖清单：涵盖框架、UI、动画、图标与AI服务等关键依赖。
