...
- [.next-joymate2/build-manifest.json](file://.next-joymate2/build-manifest.json)
- [.next-joymate2/routes-manifest.json](file://.next-joymate2/routes-manifest.json)
- [.next-joymate2/app-build-manifest.json](file://.next-joymate2/app-build-manifest.json)
- [.next-joymate/build-manifest.json](file://.next-joymate/build-manifest.json)
- [.next-joymate/routes-manifest.json](file://.next-joymate/routes-manifest.json)
- [.next-joymate/app-build-manifest.json](file://.next-joymate/app-build-manifest.json)
</cite>
...
- **更新** 当前构建系统已切换至新的构建目录结构（.next-joymate2），替代了原有的.next-joymate目录，提供更清晰的构建产物组织和简化的构建清单系统。
...
A --> K[".next-joymate2 新构建目录"]
A --> L[".next-joymate 旧构建目录(已弃用)"]
```
...
- [eslint.config.mjs](file://eslint.config.mjs)
- [.codex/environments/environment.toml](file://.codex/environments/environment.toml)
- [.next-joymate2](file://.next-joymate2)
- [.next-joymate](file://.next-joymate)
...
- **更新** 构建目录结构：当前使用.next-joymate2作为主要构建输出目录，包含完整的构建清单和静态资源，替代了旧的.next-joymate目录。
...
- **更新** 构建目录切换：项目已从.next-joymate迁移到.next-joymate2，新目录包含更完整的构建清单和优化的静态资源组织。
...
Next->>Build : 生成构建产物(.next-joymate2)
Dev->>Start : 生产构建(npm run build)
Start->>Build : 读取构建产物(.next-joymate2)
...
- **更新** 构建清单系统：.next-joymate2目录包含简化的构建清单，移除了.fallback-build-manifest.json等冗余文件，提供更清晰的构建信息组织。
...
- **更新** 构建清单优化：新的.next-joymate2目录提供简化的构建清单系统，移除冗余文件，提升构建效率和可维护性。
...
- **更新** 构建目录迁移：如果遇到构建目录相关问题，确认使用正确的.next-joymate2目录而非遗留的.next-joymate目录。
...
## 结论
本文件基于仓库现有配置与脚本，提供了从环境准备、构建与启动、容器化与云部署、环境变量管理到监控与CI/CD的完整运维指南。**更新** 重点反映了新的构建目录结构（.next-joymate2）和简化的构建清单系统，这些变更提升了构建效率和可维护性。建议在实际落地时结合团队规范与平台特性进行细化与扩展。

## 附录
- 本地运行与部署步骤：参考项目README中的运行与部署说明，确保前置条件与环境变量正确配置。
- 环境入口：通过.Codex环境配置提供一键运行入口，简化本地开发体验。
- **更新** 构建目录对比：.next-joymate2相比旧的.next-joymate目录，提供了更清晰的文件组织和简化的构建清单，移除了冗余的fallback构建清单文件。
